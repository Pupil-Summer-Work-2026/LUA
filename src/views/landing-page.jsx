...
const articles = [
  ['/placeholderbilde1231-g9db-200h.png','c1','A good title','This is how the text will look'],
  ['/placeholderbilde1231-g9db-200h.png','c2','A working title','We are still writing'],
  ['/placeholderbilde1231-g9db-200h.png', 'c3', 'Simple title', 'Waiting...'],
  ['/placeholderbilde1231-g9db-200h.png', 'c4', 'Another title', 'Experimenting with text is not forbides'],
  ['/placeholderbilde1231-g9db-200h.png', 'c5', 'yup', 'text example']
]
  ...
  return (
    <SiteLayout className="lua-home">
      ...
      <main>
        ...
        <section className="lua-hero"><div className="lua-hero__content"><h1>LATVIJAS<br /><em>UGUNSDROŠĪBAS</em><br />ASOCIĀCIJA</h1><p>Latvijas Ugunsdrošības asociācija apvieno ugunsdrošības un ugunsdzēsības jomas uzņēmumus un speciālistus, veicinot drošu vidi visā valstī.</p><button onClick={() => history.push('/ktparbiedru')}>Piesakieties</button></div></section>
        
        <section className="lua-statistics" aria-label="Asociācijas rādītāji">{[['30', 'BIEDRI'], ['4', 'SADARBĪBAS']].map(([number, label]) => <div key={label}><strong>{number}</strong><span>{label}</span></div>)}</section>
        
        <section className="lua-section lua-container lua-intro"><div className="lua-intro__copy"><span className="lua-eyebrow">PAR ASOCIĀCIJU</span><h2>DROŠĀKA<br /><em>LATVIJA</em><br />KOPĀ</h2><p>Latvijas Ugunsdrošības asociācija (LUA) ir dibināta 2002. gadā un apvieno Latvijas ugunsdrošības un ugunsdzēsības jomas uzņēmumus, organizācijas un speciālistus. Mēs sekmējam nozares vienotību, sadarbību un ilgtspēju. Mūsu asociācija aktīvi sadarbojas ar valsts iestādēm, lai veidotu efektīvu ugunsdrošības politiku visā Latvijā.</p><button className="lua-text-link" onClick={() => history.push('/about-us')}>Uzzināt vairāk <span>→</span></button></div><div className="lua-intro__visual"><span className="lua-intro__backdrop" aria-hidden="true" /><img className="lua-intro__image" src="/image49345-k5nh-600w.png" alt="Ugunsdzēšamie aparāti" /></div></section>
        ...
        <section className="lua-section lua-container lua-news"><div className="lua-section__heading"><div><span className="lua-eyebrow">JAUNUMI</span><h2>ATKLĀJIET JAUNĀKĀS IZMAIŅAS</h2></div><button className="lua-text-link" onClick={() => history.push('/jaunumi')}> Citi jaunumi <span>→</span></button></div><div className="lua-article-grid">{articles.map(([image,category,title,text]) => <Link key={image} to="/jaunums"><img src={image} alt="Jaunuma attēls" /><div><span>{category}</span><h3>{title}</h3><p>{text}</p></div></Link>)}</div></section>
        
        <section className="lua-community"><div className="lua-section lua-container"><span className="lua-eyebrow">BIEDRI UN PARTNERI</span><h2>MŪSU NOZARES <em>KOPIENA</em></h2><h3>BIEDRU UZŅĒMUMI</h3><div className="lua-logo-carousel" role="region" aria-label="Biedru uzņēmumu logo"><div className="lua-logo-carousel__track">{[0, 1].map((copyIndex) => <div className="lua-logo-carousel__group" aria-hidden={copyIndex === 1} key={copyIndex}>{featuredMemberLogos.map((logo) => <div className="lua-logo-carousel__item" key={`${copyIndex}-${logo}`}><img src={logo} alt={copyIndex === 0 ? 'Biedra logo' : ''} /></div>)}</div>)}</div></div><h3>SADARBĪBAS PARTNERI</h3><div className="lua-logo-grid lua-logo-grid--empty">{Array.from({ length: 4 }, (_, index) => <div key={index} />)}</div></div></section>
        
        <section className="lua-join"><div><h2>PIETEIKTIES LUA</h2><p>Pievienojieties vairāk nekā 30 uzņēmumiem, lai kopā veidotu drošāku Latviju.</p></div><button onClick={() => history.push('/ktparbiedru')}>Kļūt par biedru</button></section>
      </main>
    </SiteLayout>
  )
}
