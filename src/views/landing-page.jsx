  ...
  return (
    <SiteLayout className="lua-home">
      ...
      <main>
        ...
        <section className="lua-community"><div className="lua-section lua-container"><span className="lua-eyebrow">BIEDRI UN PARTNERI</span><h2>MŪSU NOZARES<br /><em>KOPIENA</em></h2><h3>BIEDRU UZŅĒMUMI</h3><div className="lua-logo-carousel" role="region" aria-label="Biedru uzņēmumu logo"><div className="lua-logo-carousel__track">{[0, 1].map((copyIndex) => <div className="lua-logo-carousel__group" aria-hidden={copyIndex === 1} key={copyIndex}>{members.map(([logo, title, link, hasWhiteBackground], index) => link ? <a className="lua-logo-carousel__item" href={link} target="_blank" rel="noreferrer" tabIndex={copyIndex === 1 ? -1 : undefined} key={`${copyIndex}-${title}-${index}`}><img className={hasWhiteBackground ? 'logo--blend-background' : undefined} src={logo} alt={copyIndex === 0 ? `${title} logotips` : ''} /></a> : <div className="lua-logo-carousel__item" key={`${copyIndex}-${title}-${index}`}><img className={hasWhiteBackground ? 'logo--blend-background' : undefined} src={logo} alt={copyIndex === 0 ? `${title} logotips` : ''} /></div>)}</div>)}</div></div><h3>SADARBĪBAS PARTNERI</h3><div className="lua-logo-grid">{[['/Biedri/DAKIB.png', 'DAKIB', 'https://www.dakib.lv/', true], ['/Biedri/iekslietu ministrija.png', 'Iekšlietu ministrija', 'https://www.iem.gov.lv/lv'], ['/Biedri/LDDK.svg', 'Latvijas Darba devēju konfederācija', 'https://lddk.lv/'], ['/Biedri/vugd.png', 'Valsts ugunsdzēsības un glābšanas dienests', 'https://www.vugd.gov.lv/lv', true]].map(([logo, name, link, hasWhiteBackground]) => <a className="lua-logo-grid__item" href={link} target="_blank" rel="noreferrer" key={name}><img className={hasWhiteBackground ? 'logo--blend-background' : undefined} src={logo} alt={`${name} logotips`} /></a>)}</div></div></section>
